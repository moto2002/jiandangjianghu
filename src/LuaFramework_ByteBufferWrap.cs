...
public class LuaFramework_ByteBufferWrap
{
	public static void Register(LuaState L)
	{
		...
		L.RegFunction("ReadPbBuffer", new LuaCSFunction(LuaFramework_ByteBufferWrap.ReadPbBuffer));
		...
		L.RegVar("Length", new LuaCSFunction(LuaFramework_ByteBufferWrap.get_Length), null);
		L.EndClass();
	}
	...
	[MonoPInvokeCallback(typeof(LuaCSFunction))]
	private static int ReadPbBuffer(IntPtr L)
	{
		int result;
		try
		{
			ToLua.CheckArgsCount(L, 1);
			ByteBuffer byteBuffer = (ByteBuffer)ToLua.CheckObject(L, 1, typeof(ByteBuffer));
			LuaByteBuffer bb = byteBuffer.ReadPbBuffer();
			ToLua.Push(L, bb);
			result = 1;
		}
		catch (Exception e)
