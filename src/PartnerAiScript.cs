...
public class PartnerAiScript : MonoBehaviour
{
	private int PET_DISTANCE = 5;

	private int PET_MAX_DISTANCE = 8;

	private int PET_AUTO_WALK_TIME = 5;
	...
	private void Update()
	{
		...
		if (this.self.roleAction.curNextStatus == 2 || this.self.roleAction.curNextStatus == 20 || this.self.roleAction.curNextStatus == 6)
		{
			this.curInterval = Time.time;
			return;
		}
		...
		if ((this.owner.roleAction.curStatus < 6 || this.owner.roleAction.curStatus > 14) && this.self.roleAction.curStatus == 6)
		{
			this.self.roleAction.SetRoleIdle();
			return;
		}
	}
