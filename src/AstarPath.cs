...
[AddComponentMenu("Pathfinding/Pathfinder"), ExecuteInEditMode, HelpURL("http://arongranberg.com/astar/docs/class_astar_path.php")]
public class AstarPath : MonoBehaviour
{
	...
	{
		AstarPath.<DelayedGraphUpdate>c__Iterator9 <DelayedGraphUpdate>c__Iterator = new AstarPath.<DelayedGraphUpdate>c__Iterator9();
	...
	{
		AstarPath.<UpdateGraphsInteral>c__IteratorA <UpdateGraphsInteral>c__IteratorA = new AstarPath.<UpdateGraphsInteral>c__IteratorA();
		<UpdateGraphsInteral>c__IteratorA.t = t;
		<UpdateGraphsInteral>c__IteratorA.ob = ob;
		<UpdateGraphsInteral>c__IteratorA.<$>t = t;
		<UpdateGraphsInteral>c__IteratorA.<$>ob = ob;
		<UpdateGraphsInteral>c__IteratorA.<>f__this = this;
		return <UpdateGraphsInteral>c__IteratorA;
	}
	...
	public void ScanLoop(OnScanStatus statusCallback)
	{
		...
		for (i = 0; i < this.graphs.Length; i++)
		{
			...
			if (navGraph == null)
			{
				if (statusCallback != null)
				{
					statusCallback(new Progress(Mathf.Lerp(0.05f, 0.7f, ((float)i + 0.5f) / (float)(this.graphs.Length + 1)), string.Concat(new object[]
					{
						"Skipping graph ",
						i + 1,
						" of ",
			...
			else
			{
				if (AstarPath.OnGraphPreScan != null)
				{
					if (statusCallback != null)
					{
						statusCallback(new Progress(Mathf.Lerp(0.1f, 0.7f, (float)i / (float)this.graphs.Length), string.Concat(new object[]
						{
							"Scanning graph ",
							i + 1,
							" of ",
				...
				if (AstarPath.OnGraphPostScan != null)
				{
					if (statusCallback != null)
					{
						statusCallback(new Progress(Mathf.Lerp(0.1f, 0.7f, ((float)i + 0.95f) / (float)this.graphs.Length), string.Concat(new object[]
						{
							"Scanning graph ",
							i + 1,
							" of ",
	...
	{
		AstarPath.<CalculatePaths>c__IteratorB <CalculatePaths>c__IteratorB = new AstarPath.<CalculatePaths>c__IteratorB();
		<CalculatePaths>c__IteratorB._threadInfo = _threadInfo;
		<CalculatePaths>c__IteratorB.<$>_threadInfo = _threadInfo;
		return <CalculatePaths>c__IteratorB;
	}
