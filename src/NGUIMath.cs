...
public static class NGUIMath
{
	...
	public static Rect ConvertToTexCoords(Rect rect, int width, int height)
	{
		...
		if ((float)width != 0f && (float)height != 0f)
		{
			...
			result.xMin = rect.xMin / (float)width + num;
			result.xMax = rect.xMax / (float)width - num;
			result.yMin = 1f - rect.yMax / (float)height + num2;
			result.yMax = 1f - rect.yMin / (float)height - num2;
		}
		return result;
	}
