...
public class UnityEngine_TextureWrap
{
	...
	private static int set_anisotropicFiltering(IntPtr L)
	{
		...
		try
		{
			AnisotropicFiltering anisotropicFiltering = (AnisotropicFiltering)((int)ToLua.CheckObject(L, 2, typeof(AnisotropicFiltering)));
	...
	private static int set_filterMode(IntPtr L)
	{
		...
		try
		{
			obj = ToLua.ToObject(L, 1);
			Texture texture = (Texture)obj;
			FilterMode filterMode = (FilterMode)((int)ToLua.CheckObject(L, 2, typeof(FilterMode)));
	...
	private static int set_wrapMode(IntPtr L)
	{
		...
		try
		{
			obj = ToLua.ToObject(L, 1);
			Texture texture = (Texture)obj;
			TextureWrapMode wrapMode = (TextureWrapMode)((int)ToLua.CheckObject(L, 2, typeof(TextureWrapMode)));
