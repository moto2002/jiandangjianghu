...
public class UIWidgetWrap
{
	...
	private static int WriteToBuffers(IntPtr L)
	{
		...
		try
		{
			...
			BetterList<Color32> c = (BetterList<Color32>)ToLua.CheckObject(L, 4, typeof(BetterList<Color32>));
	...
	private static int set_keepAspectRatio(IntPtr L)
	{
		...
		try
		{
			obj = ToLua.ToObject(L, 1);
			UIWidget uIWidget = (UIWidget)obj;
			UIWidget.AspectRatioSource keepAspectRatio = (UIWidget.AspectRatioSource)((int)ToLua.CheckObject(L, 2, typeof(UIWidget.AspectRatioSource)));
