...
public class UnityEngine_AudioSourceWrap
{
	public static void Register(LuaState L)
	{
		...
		L.RegFunction("DOPitch", new LuaCSFunction(UnityEngine_AudioSourceWrap.DOPitch));
		L.RegFunction("DOFade", new LuaCSFunction(UnityEngine_AudioSourceWrap.DOFade));
		...
		L.RegVar("isVirtual", new LuaCSFunction(UnityEngine_AudioSourceWrap.get_isVirtual), null);
	...
	[MonoPInvokeCallback(typeof(LuaCSFunction))]
	private static int DOPitch(IntPtr L)
	{
		int result;
		try
		{
			ToLua.CheckArgsCount(L, 3);
			AudioSource target = (AudioSource)ToLua.CheckObject(L, 1, typeof(AudioSource));
			float endValue = (float)LuaDLL.luaL_checknumber(L, 2);
			float duration = (float)LuaDLL.luaL_checknumber(L, 3);
			Tweener o = target.DOPitch(endValue, duration);
	...
	private static int get_isVirtual(IntPtr L)
	{
		...
		{
			result = LuaDLL.toluaL_exception(L, ex, (obj != null) ? ex.Message : "attempt to index isVirtual on a nil value");
		}
		return result;
	}
