...
public class UnityEngine_AssetBundleWrap
{
	public static void Register(LuaState L)
	{
		...
		L.RegVar("isStreamedSceneAssetBundle", new LuaCSFunction(UnityEngine_AssetBundleWrap.get_isStreamedSceneAssetBundle), null);
		L.EndClass();
	}
	...
	private static int LoadFromFileAsync(IntPtr L)
	{
		...
		try
		{
			...
			else if (num == 3 && TypeChecker.CheckTypes(L, 1, typeof(string), typeof(uint), typeof(ulong)))
			{
				string path3 = ToLua.ToString(L, 1);
				uint crc2 = (uint)LuaDLL.lua_tonumber(L, 2);
				ulong offset = LuaDLL.tolua_touint64(L, 3);
	...
	private static int LoadFromFile(IntPtr L)
	{
		...
		try
		{
			...
			else if (num == 3 && TypeChecker.CheckTypes(L, 1, typeof(string), typeof(uint), typeof(ulong)))
			{
				...
				uint crc2 = (uint)LuaDLL.lua_tonumber(L, 2);
				ulong offset = LuaDLL.tolua_touint64(L, 3);
				AssetBundle obj3 = AssetBundle.LoadFromFile(path3, crc2, offset);
				ToLua.Push(L, obj3);
				result = 1;
			}
			else
	...
	private static int get_isStreamedSceneAssetBundle(IntPtr L)
	{
		...
		{
			result = LuaDLL.toluaL_exception(L, ex, (obj != null) ? ex.Message : "attempt to index isStreamedSceneAssetBundle on a nil value");
		}
		return result;
	}
}
