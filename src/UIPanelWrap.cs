...
public class UIPanelWrap
{
	...
	private static int set_renderQueue(IntPtr L)
	{
		...
		try
		{
			obj = ToLua.ToObject(L, 1);
			UIPanel uIPanel = (UIPanel)obj;
			UIPanel.RenderQueue renderQueue = (UIPanel.RenderQueue)((int)ToLua.CheckObject(L, 2, typeof(UIPanel.RenderQueue)));
	...
	private static int set_clipping(IntPtr L)
	{
		...
		try
		{
			obj = ToLua.ToObject(L, 1);
			UIPanel uIPanel = (UIPanel)obj;
			UIDrawCall.Clipping clipping = (UIDrawCall.Clipping)((int)ToLua.CheckObject(L, 2, typeof(UIDrawCall.Clipping)));
