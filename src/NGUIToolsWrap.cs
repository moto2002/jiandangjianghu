...
public class NGUIToolsWrap
{
	public static void Register(LuaState L)
	{
		...
		L.RegFunction("MarkParentAsChangedEx", new LuaCSFunction(NGUIToolsWrap.MarkParentAsChangedEx));
	...
	private static int AddSprite(IntPtr L)
	{
		...
		try
		{
			...
			UISprite obj = NGUITools.AddSprite(go, atlas, spriteName);
			ToLua.Push(L, obj);
			result = 1;
		}
		catch (Exception e)
	...
	[MonoPInvokeCallback(typeof(LuaCSFunction))]
	private static int MarkParentAsChangedEx(IntPtr L)
	{
		int result;
		try
		{
			ToLua.CheckArgsCount(L, 1);
			GameObject go = (GameObject)ToLua.CheckUnityObject(L, 1, typeof(GameObject));
			NGUITools.MarkParentAsChangedEx(go);
			result = 0;
		}
		catch (Exception e)
		{
