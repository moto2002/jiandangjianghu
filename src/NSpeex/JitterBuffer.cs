...
namespace NSpeex
{
	public class JitterBuffer
	{
		...
		public int Get(ref JitterBuffer.JitterBufferPacket packet, int desired_span, out int start_offset)
		{
			...
			for (i = 0; i < 200; i++)
			{
				if (this.packets[i].data != null && this.packets[i].timestamp == this.pointer_timestamp && this.packets[i].timestamp + this.packets[i].span >= this.pointer_timestamp + (long)desired_span)
				{
					break;
				}
			}
			...
			if (i == 200)
			{
				for (i = 0; i < 200; i++)
				{
					if (this.packets[i].data != null && this.packets[i].timestamp <= this.pointer_timestamp && this.packets[i].timestamp + this.packets[i].span > this.pointer_timestamp)
					{
						break;
					}
				}
