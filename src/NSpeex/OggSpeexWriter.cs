...
namespace NSpeex
{
	public class OggSpeexWriter : AudioFileWriter
	{
		...
		public override void WritePacket(byte[] data, int offset, int len)
		{
			...
			this.packetCount++;
			this.granulepos += (long)(this.nframes * ((this.mode != 2) ? ((this.mode != 1) ? 160 : 320) : 640));
		}

		private void Flush(bool eos)
		{
			OggCrc oggCrc = new OggCrc();
			byte[] array = OggSpeexWriter.BuildOggPageHeader((!eos) ? 0 : 4, this.granulepos, this.streamSerialNumber, this.pageCount++, this.packetCount, this.headerBuffer);
			oggCrc.Initialize();
			oggCrc.TransformBlock(array, 0, array.Length, array, 0);
