...
public class UITexture : UIBasicSprite
{
	...
	public Texture cacheTexture
	{
		get
		{
			if (this.mCacheTexture == null && !string.IsNullOrEmpty(this.texturePath))
			{
				if (Application.isPlaying)
				{
					string assetName = this.texturePath.Replace("Assets/Resources/", string.Empty);
					ResourceManager manager = AppFacade.Instance.GetManager<ResourceManager>("ResourceManager");
					if (manager != null)
					{
						this.mainTexture = manager.LoadTexture(assetName);
					}
				}
			}
			if (this.mCacheTexture == null)
			{
				this.mCacheTexture = this.mTexture;
			}
			return this.mCacheTexture;
		}
		set
		{
			this.mCacheTexture = value;
		}
	}
	...
	public override Vector4 drawingDimensions
	{
		get
		{
			...
			if (this.mCacheTexture != null && this.mType != UIBasicSprite.Type.Tiled)
			{
				int width = this.mCacheTexture.width;
				int height = this.mCacheTexture.height;
				int num5 = 0;
				int num6 = 0;
				float num7 = 1f;
				float num8 = 1f;
	...
	private void addTextureCache()
	{
		...
		string key = this.texturePath.Replace("Assets/Resources/", string.Empty);
		if (!UITexture.textureList.ContainsKey(key))
		{
			UITexture.textureList[key] = new BetterList<UITexture>();
		}
		if (!UITexture.textureList[key].Contains(this))
		{
			UITexture.textureList[key].Add(this);
		}
		if (this.cacheTexture == null)
		{
			Debug.LogWarning("<<UITexture , cacheTexture>> Load Texture Faile--->" + this.texturePath);
		}
	}
	...
	public void Load(string path)
	{
		...
		if (manager != null)
		{
			this.mainTexture = manager.LoadTexture(this.texturePath);
		}
		this.addTextureCache();
	}
	...
	public void LoadResources(string path)
	{
		...
		if (manager != null)
		{
			this.mainTexture = manager.LoadTexture(this.texturePath, false);
		}
		this.addTextureCache();
	}
	...
	protected override void OnDisable()
	{
		...
		if (!string.IsNullOrEmpty(this.texturePath))
		{
			...
			if (UITexture.textureList.ContainsKey(key))
			{
				if (UITexture.textureList[key].Contains(this))
				{
					UITexture.textureList[key].Remove(this);
				}
				if (UITexture.textureList[key].size <= 0)
				{
					UITexture.textureList.Remove(key);
				}
			}
		}
	}
	...
	{
		return UITexture.textureList.ContainsKey(relativePath) && UITexture.textureList[relativePath].size > 0;
	}
}
