...
namespace Pathfinding
{
	...
	public class RecastGraph : NavGraph, IUpdatableGraph, IRaycastableGraph, INavmesh, INavmeshHolder
	{
		...
		{
			List<ExtraMesh> list;
			this.CollectMeshes(out list, new Bounds(Vector3.zero, new Vector3(float.PositiveInfinity, float.PositiveInfinity, float.PositiveInfinity)));
			if (list.Count == 0)
			{
				return;
			}
		...
		public void GetRecastMeshObjs(Bounds bounds, List<ExtraMesh> buffer)
		{
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				else
				{
					...
					if (collider == null)
					{
						Debug.LogError("RecastMeshObject (" + list[i].gameObject.name + ") didn't have a collider or MeshFilter+Renderer attached");
					}
					else
					{
		...
		public void UpdateArea(GraphUpdateObject guo)
		{
			...
			{
				for (int l = a.ymin; l <= a.ymax; l++)
				{
					this.RemoveConnectionsFromTile(this.tiles[k + l * this.tileXCount]);
				}
			}
			for (int m = a.xmin; m <= a.xmax; m++)
			{
				...
				{
					this.BuildTileMesh(voxelize, m, n);
				}
			}
			...
			for (int num = a.xmin; num <= a.xmax; num++)
			{
				for (int num2 = a.ymin; num2 <= a.ymax; num2++)
				{
					...
					{
						nodes[num3].GraphIndex = graphIndex;
					}
				}
			}
			a = a.Expand(1);
			...
			for (int num4 = a.xmin; num4 <= a.xmax; num4++)
			{
				...
				{
					if (num4 < this.tileXCount - 1 && a.Contains(num4 + 1, num5))
					{
						this.ConnectTiles(this.tiles[num4 + num5 * this.tileXCount], this.tiles[num4 + 1 + num5 * this.tileXCount]);
					}
					if (num5 < this.tileZCount - 1 && a.Contains(num4, num5 + 1))
					{
						this.ConnectTiles(this.tiles[num4 + num5 * this.tileXCount], this.tiles[num4 + (num5 + 1) * this.tileXCount]);
					}
				}
			}
		}
			...
			for (int k = 0; k < num4; k++)
			{
				for (int l = 0; l < num3; l++)
				{
					...
					if (statusCallback != null && (num6 * 10 / this.tiles.Length > num5 || stopwatch.ElapsedMilliseconds > 2000L))
					{
						...
						stopwatch.Reset();
						stopwatch.Start();
						statusCallback(new Progress(Mathf.Lerp(0.1f, 0.9f, (float)num6 / (float)this.tiles.Length), string.Concat(new object[]
						{
							"Building Tile ",
							num6,
							"/",
